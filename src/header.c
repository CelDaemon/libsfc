...
bool sfc_header_title(const sfc_header *header, char title[SFC_MAX_TITLE_LENGTH + 1]) {
    assert(header != NULL);
    assert(title != NULL);
    if (!valid_title(TITLE(header), SFC_MAX_TITLE_LENGTH)) {
        errno = EINVAL;
        return false;
    }
    memcpy(title, TITLE(header), SFC_MAX_TITLE_LENGTH);
...
bool sfc_header_set_title(sfc_header *header, const char *title) {
    ...
    memcpy(TITLE(header), title, length);
    memset(TITLE(header) + length, ' ', SFC_MAX_TITLE_LENGTH - length);
    return true;
}

enum sfc_speed sfc_header_speed(const sfc_header *header) {
    assert(header != NULL);
    return (ROM_MODE(header) & 0x10) > 0 ? SFC_SPD_FAST : SFC_SPD_SLOW;
}
...
bool sfc_header_set_speed(sfc_header *header, const enum sfc_speed speed) {
    ...
    ROM_MODE(header) = bit_set(ROM_MODE(header), 4, speed == SFC_SPD_FAST ? 1 : 0);
    return true;
}

enum sfc_map sfc_header_map(const sfc_header *header) {
    assert(header != NULL);
    switch (ROM_MODE(header) & 0xF) {
        case 0:
            return SFC_MAP_LO;
        case 1:
            return SFC_MAP_HI;
...
bool sfc_header_set_map(sfc_header *header, const enum sfc_map map) {
    ...
    ROM_MODE(header) = (ROM_MODE(header) & ~0xF) | value;
    return true;
}

enum sfc_chipset sfc_header_chipset(const sfc_header *header) {
    assert(header != NULL);
    switch (CHIPSET(header) & 0xF) {
        case 0x00:
            return 0;
        case 0x01:
            return SFC_CHP_RAM;
...
bool sfc_header_set_chipset(sfc_header *header, const enum sfc_chipset chipset) {
    ...
    CHIPSET(header) = (uint8_t) ((CHIPSET(header) & 0xF) | value);
    return true;
...
void sfc_header_set_checksum(sfc_header *header, const uint16_t checksum) {
    ...
    CHECKSUM_COMPLEMENT(header) = ~checksum;
}